...
BZ_CONTROL_MAX = 900						#hope it is enough!
...
file__ = __FILE__.dup
if defined?(Encoding) 
	#if file__.encoding.inspect =~ /Windows/i
	#	file__ = file__.dup.force_encoding('iso-8859-1').encode("UTF-8")
	#elsif file__.encoding.inspect !~ /UTF/i
		file__ = __FILE__.dup.force_encoding("UTF-8")
	#end	
end	
file__ = file__.gsub(/\\/, '/')
...
def get_method(methodname, arity)
	...
	begin
		...
		#puts "undefined method #{methodname} for #{@modulename} in file #{@filename}"
		return nil
	end
	
	unless (hmeth.arity >= arity)
		puts "incorrect number of arguments for method #{methodname} in #{@modulename} in file #{@filename}"
		return nil
	end
...
def BezierMathsCurve.is_selection_a_curve
    ss = Sketchup.active_model.selection
	#return nil if ss.length > 200
	hcurve = {}
	ledges = ss.grep(Sketchup::Edge)
	ledges.each do |e|
		curve = e.curve
...
def this_file_name	
	...
	if defined?(Encoding) 
		...
		#	file__ = file__.dup.force_encoding('iso-8859-1').encode("UTF-8")
		#elsif file__.encoding.inspect !~ /UTF/i
			file__ = file__.dup.force_encoding("UTF-8")
		#end	
	end	
	file__ = file__.gsub(/\\/, '/')
	file__ =~ /\w+\s*\w+\.rb/i
...
def load_one_module(filename, modulename)
	begin
		...
		puts "undefined module #{modulename} in file #{filename}"
		return nil
	end	
...
def compute_contextual_menu(menu)
	return if Sketchup.active_model.selection.length > 1000
...
def compute_icons(cmd, name_icon, tooltip)
	retina_factor = (defined?(UI.scale_factor)) ? UI.scale_factor : 1.0
	...
	icon_32 = check_icon(name_icon + '_32.png')
	icon = check_icon(name_icon + '.png')
	small_icon = (retina_factor >= 1.25) ? icon_24 : icon_16
	small_icon = icon unless small_icon
	small_icon = icon_24 unless small_icon
	large_icon = (retina_factor >= 1.25) ? icon_32 : icon_24
...
def check_icon(name)
	#Trying first SVG or PDF
	ext = (Traductor.platform_is_mac?) ? "pdf" : "svg"
	name_vec = name.sub(/png\Z/i, ext)
	f = File.join(BZ___DirBZ, BZ_IMAGE_Folder, name_vec)
...
def Bezierspline.close_loop_by_Bezier(pts, numseg, crvpts)
	...
	ptlast = Geom::Point3d.new *crvpts[n]
	loopts[0] = ptlast
	ptprev = crvpts[n-1]
	...
	ptlast = Geom::Point3d.new *crvpts[0]
	loopts[3] = ptlast
	ptprev = crvpts[1]	
	vector = ptprev.vector_to ptlast 
...
#-----------------------------------------------------------------
class DialogBox
	unless defined?(DlgItem)
		DlgItem = Struct.new(:symb, :type, :label, :default, :vmin, :vmax,
