...
# TYPE      : SketchUp 2026 Main Plugin Script
...
#   - `../ValeDesignSuite_LoaderScript.rb` (relative from plugin root directory)
#
# USER INTERFACE:
...
#
# 03-Sep-2025 - Version 0.0.8 - ADDED PATH TO CONTINUOUS PROFILE TOOL
# - Added automatic loading of VDS__Tool__Create3d__PathToContinousProfileTool
# - Tool now loads on plugin initialization with menu items registered
# - Supports both file-by-file and directory batch processing modes
#
# 16-Sep-2025 - Version 0.0.9 - ADDED EXPORT WHITECARD SCENES TOOL
# - Added automatic loading of VDS__Utils__ExportWhitecardScenes
# - Integrated export automation tool into main UI
# - Added button to launch whitecard export workflow
# - Supports PNG image and DWG CAD file exports with standardized settings
#
# =============================================================================
...
module ValeDesignSuite
    ...
    def self.initialize_plugin
        ...
        # -------------------------------------------------------------
        # Added in Version 0.0.8 - 03-Sep-2025
        begin
            require_relative 'Tools_RoofLanternConfigurator/ValeDesignSuite_Tools_RoofLanternTools__3.0.0'
        ...
        # LOAD MODULE |  Load & Validate the Watercolor Rendering Tools Module
        # -------------------------------------------------------------
        begin
            require_relative 'Tools_ImageRenderingTools/ValeDesignSuite_RenderingTools__WatercolourRenderer'
            Config::DebugConfiguration.debug_log("Successfully loaded WatercolourRenderingTools", "LOAD")
        rescue LoadError => e
        ...
        # -------------------------------------------------------------
        begin
            require_relative 'Tools_RoofLanternConfigurator/VDS__Tool__Create3d__PathToContinousProfileTool'
            Config::DebugConfiguration.debug_log("Successfully loaded PathToContinuousProfileTool", "LOAD")
        rescue LoadError => e
            Config::DebugConfiguration.debug_error("Failed to load PathToContinuousProfileTool", e)
        end

        # LOAD MODULE |  Load & Validate the Export Whitecard Scenes Tool
        # -------------------------------------------------------------
        begin
            require_relative 'Utils__CustomFileExportAutomation/VDS__Utils__ExportWhitecardScenes'
            Config::DebugConfiguration.debug_log("Successfully loaded ExportWhitecardScenes", "LOAD")
        rescue LoadError => e
            Config::DebugConfiguration.debug_error("Failed to load ExportWhitecardScenes", e)
        end

        # LOAD MODULE |  Load & Validate the Notes App Module
        # -------------------------------------------------------------
        begin
            require_relative 'Utils__NotesAppAndImageViewer/Util__SketchUpModel__InBuiltNotesApp__HtmlDialogue'
            Config::DebugConfiguration.debug_log("Successfully loaded NotesApp", "LOAD")
        rescue LoadError => e
            Config::DebugConfiguration.debug_error("Failed to load NotesApp", e)
        end

        # LOAD MODULE |  Load & Validate the Image Carousel Module
        # -------------------------------------------------------------
        begin
            require_relative 'Utils__NotesAppAndImageViewer/Util__SketchUpModel__InBuiltImageViewingCaraselApp__HtmlDialogue'
            Config::DebugConfiguration.debug_log("Successfully loaded ImageCarousel", "LOAD")
        rescue LoadError => e
            Config::DebugConfiguration.debug_error("Failed to load ImageCarousel", e)
        end
  ...
  unless file_loaded?(__FILE__)
    ...
    # DEVELOPMENT UTILITY | Expose script reload via skp: URL bridge
    # -------------------------------------------------------------------------
    # Reload all unencrypted Ruby source files under the ValeDesignSuite plugin directory during development. 
    # This forces Ruby to re-evaluate each `.rb` file (using `load`), refreshes inspectors, and rebuilds the main HtmlDialog
    # so UI changes take effect immediately. 
    # Note : Encrypted files (`.rbe/.rbs`) are not reloaded and require a SketchUp restart. Intended for development use only.

         def self.reload_vale_design_suite_scripts
         begin
         count = ValeDesignSuite::MainUserInterface.reload_vale_design_suite_scripts
         Config::DebugConfiguration.debug_log("Reloaded #{count} ValeDesignSuite script(s)", "RELOAD")
         rescue => e
         Config::DebugConfiguration.debug_error("Reload failed", e)
