...
/**
 * @return {string} Location of the temp directory.
 */
skp.tempPath = function() {
  return su.info['temp_path'];
};
...
su.isFunction = function (suspect) {
    ...
    return typeof suspect == 'function' &&
        (suspect.toString().replace(/^[\s\xa0]+|[\s\xa0]+$/g, "")
            .indexOf('function') == 0);
};
...
/**
 * Responds to notifications that SketchUp does not have network connectivity.
 * @param {string} queryid The ID of the query used to query offline state.
 * @private
 */
su.handleIsOffline_ = function(queryid) {
  if (su.IS_ONLINE) {
    return;
  }
