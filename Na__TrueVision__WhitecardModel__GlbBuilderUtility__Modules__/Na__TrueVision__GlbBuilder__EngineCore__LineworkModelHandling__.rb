...
module TrueVision3D
    ...
    # -------------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------
        def self.Na__LineworkEngine__TraverseEdges(entities, parent_transform, parent_layer, positions, colors)
            entities.each do |entity|
                ...
                if entity.is_a?(Sketchup::Edge)
                    ...
                    Na__LineworkEngine__TraverseEdges(entity.definition.entities, child_transform, child_layer, positions, colors)
                end
            end
        end
        # ---------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
        ...
        def self.Na__LineworkEngine__BuildGltfFromEdgeData(positions, colors)
            gltf = {
                "asset"       => { "version" => "2.0", "generator" => "TrueVision3D GLB Builder Linework v1.2.0" },
                "scene"       => 0,
                "scenes"      => [{ "nodes" => [0] }],
                "nodes"       => [{ "mesh" => 0 }],
                "meshes"      => [],
    ...
    # -------------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------
        def self.Na__LineworkEngine__ExportLineworkToGlb(entities, filepath)
            ...
            begin
                ...
                if positions.empty?
                    puts "  No visible edges found - skipping linework file"
                    return false
                end

                edge_count = positions.length / 6
                puts "    Linework complete: #{entity_count} entities -> #{edge_count} edges"
