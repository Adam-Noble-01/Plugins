# =============================================================================
# NA NOBLE3D MODELLING TOOLS - ORIENT FACES TOWARD CAMERA - TRAVERSAL
# =============================================================================
#
# FILE       : Na__Noble3dModellingTools__OrientFacesTowardCamera__Traversal__.rb
# NAMESPACE  : Na__Noble3dModellingTools::Na__OrientFacesTowardCamera
# PURPOSE    : Walk selected faces and nested containers with world transforms
# CREATED    : 2026
#
# COORDINATE RULE (SketchUp's, not ours):
# In the active editing context and its parents every API coordinate is GLOBAL;
# inside any closed group or component it is LOCAL. Start from IDENTITY at the
# active context and multiply each closed container's transformation as we descend.
#
# Shared groups and components are uniquified only when a face in that branch
# actually faces away from the camera, so unselected copies stay untouched.
#
# =============================================================================
...
# -----------------------------------------------------------------------------
        ...
        NA_SELECTION_REMAP_DICTIONARY = 'Na__OrientFacesTowardCamera__TemporarySelectionRemap'.freeze
        NA_SELECTION_REMAP_KEY = 'Na__OrientFacesTowardCamera__SelectionToken'.freeze
...
# -----------------------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        def self.na_process_selection(selection, camera)
            statistics = na_empty_statistics
            identity_transformation = IDENTITY

            selection.to_a.each do |entity|
                na_process_entity(entity, camera, identity_transformation, statistics, 0, [], true)
            end
        ...
        # ------------------------------------------------------------
        def self.na_selection_requires_change?(selection, camera)
            identity_transformation = IDENTITY
            selection.to_a.any? do |entity|
                na_entity_requires_change?(entity, camera, identity_transformation, 0, [])
            end
        end
        # ------------------------------------------------------------
        ...
        def self.na_make_active_path_unique(model, selected_entities)
            ...
            begin
                active_path.length.times do |path_index|
                    ...
                    raise 'The active editing path changed while preparing face orientation.' unless container
                    raise 'Cannot orient faces inside a locked active editing path.' if container.locked?
        ...
        # HELPER FUNCTION | Process One Face or Container
        # ------------------------------------------------------------
        def self.na_process_entity(entity, camera, transformation, statistics, depth, definition_stack, count_unsupported)
            return unless na_valid_entity?(entity)

            case entity
            when Sketchup::Face
                na_process_face(entity, transformation, camera, statistics)
            when Sketchup::Group
                na_process_container(entity, camera, transformation, statistics, depth, definition_stack)
        ...
        # ------------------------------------------------------------
        def self.na_process_container(container, camera, parent_transformation, statistics, depth, definition_stack)
            if container.locked?
            ...
            child_transformation = parent_transformation * container.transformation
            if na_container_requires_change?(container, camera, child_transformation, depth, definition_stack)
                container.make_unique
        ...
        # HELPER FUNCTION | Process One Face and Record Its Outcome
        # ------------------------------------------------------------
        def self.na_process_face(face, transformation, camera, statistics)
            statistics[:visited_face_count] += 1

            case na_orient_face(face, transformation, camera)
            when :reversed
                statistics[:reversed_face_count] += 1
            when :already_facing
                statistics[:already_facing_face_count] += 1
            when :edge_on
                statistics[:edge_on_face_count] += 1
            else
...
# -----------------------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        def self.na_entity_requires_change?(entity, camera, transformation, depth, definition_stack)
            return false unless na_valid_entity?(entity)
            return na_face_requires_reverse?(entity, transformation, camera) if entity.is_a?(Sketchup::Face)
            ...
            na_container_requires_change?(entity, camera, child_transformation, depth, definition_stack)
        end
        # ------------------------------------------------------------

        # HELPER FUNCTION | Check Whether a Container Branch Requires Mutation
        # ------------------------------------------------------------
        def self.na_container_requires_change?(container, camera, child_transformation, depth, definition_stack)
        ...
        def self.na_empty_statistics
            {
                ...
                already_facing_face_count: 0,
                edge_on_face_count: 0,
