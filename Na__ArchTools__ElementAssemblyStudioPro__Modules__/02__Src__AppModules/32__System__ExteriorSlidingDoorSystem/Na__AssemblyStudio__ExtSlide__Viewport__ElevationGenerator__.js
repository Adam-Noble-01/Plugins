...
const Na__ExtSlide__ElevationGenerator = (function () {
    ...
    function na_build_one_leaf(leafX, leafY, leafWidth, leafHeight, layout, glazingColour, panelIndex, clickBucket) {
        const sg = window.Na__Viewport__SvgGenerator;
        if (!sg) return '';
        ...
        if (layout.glazeBars.enabled && glazedW > 0 && glazedH > 0) {
            svg += na_build_leaf_glazebars(glazedX, glazedY, glazedW, glazedH, layout, panelIndex, clickBucket);
        }

        if (glazedW > 0 && glazedH > 0) {
            svg += na_build_leaf_leaded(glazedX, glazedY, glazedW, glazedH, layout, panelIndex, clickBucket);
        }
    ...
    function na_build_leaf_glazebars(glassX, glassY, glassW, glassH, layout, panelIndex, clickBucket) {
        const sg = window.Na__Viewport__SvgGenerator;
    ...
    // ------------------------------------------------------------
    function na_build_leaf_leaded(glassX, glassY, glassW, glassH, layout, panelIndex, clickBucket) {
    ...
    function na_generate_sliding_svg(config) {
        ...
        svg += na_build_leaves(layout, clickBucket);
