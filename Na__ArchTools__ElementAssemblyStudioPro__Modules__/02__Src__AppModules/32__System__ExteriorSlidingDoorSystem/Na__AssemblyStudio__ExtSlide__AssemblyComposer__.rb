...
# -----------------------------------------------------------------------------
    ...
    PanelLayoutResolver  = Na__AssemblyStudio::Na__ExteriorSlidingDoorSystem::Na__PanelLayoutResolver
...
# -----------------------------------------------------------------------------
    ...
    def self.na_build_panel_mod_group(config_hash, dims, descriptor, parent_entities, door_id, materials = nil)
        ...
        # @delegate: Na__AssemblyStudio__ExtSlide__PanelLayoutResolver__.rb
        ...
        DebugTools.na_debug_geometry(
            "ExtSlide MOD: #{mod_name} (leaf #{descriptor[:index]}, role=#{descriptor[:role]}, composition=#{panel_layout[:composition]})"
        )
        mod_group
    end
    ...
    # HELPER FUNCTION | Build Fielded Leaf Stiles / Rails With FuseParts Names
    # ------------------------------------------------------------
    def self.na_build_fielded_leaf_members(entities, leaf, layout, part_names, material)
        ...
        na_create_box_mm(entities, "Na_DoorPanel_#{part_names[:glazing].sub(/^Na_Glass_/, '')}_Rail_Mid",
    ...
    def self.na_build_fielded_field_dividers(entities, leaf, layout, material, panel_id)
        layout[:field_dividers].each do |divider|
            ...
            name = format('Na__ExteriorSlidingDoor__%s__%s%02d', panel_id, role, divider[:index])
