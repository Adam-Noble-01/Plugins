...
# =============================================================================
# ELEMENT ASSEMBLY STUDIO PRO - EXTERIOR DOOR COMMON - DXF EXPORTER
# =============================================================================
#
# FILE       : Na__AssemblyStudio__ExtDoorCommon__DxfExporter__.rb
# NAMESPACE  : Na__AssemblyStudio::Na__ExteriorDoorCommon
# MODULE     : Na__DxfExporter
# AUTHOR     : Noble Architecture
# PURPOSE    : Prefix-parameterised DXF entity builder shared by every hinged
#              exterior door product. Emits the elevation (frame, cill, leaf
#              members, fielded panels, glazing, glaze bars, handle, dimension
#              text) plus the plan block (closed footprint, open-state outline,
#              swing arc) below it.
#
# DESCRIPTION:
# - Extracted verbatim from the Exterior Double Door exporter so the double
#   door and the single door emit identical layers and geometry from one code
#   path. Only the key prefix, the product label, the resolvers and the
#   handle rule are supplied by the caller.
#
# PRODUCT CONTRACT (Hash):
#   :prefix           String  Key prefix, no trailing underscore.
#   :label            String  Title text used in the dimension note.
#   :leaf_resolver    Module  Responds to na_resolve(config) -> [leaf].
#   :panel_resolver   Module  Responds to na_resolve(config, leaf) -> panel.
#   :leaf_count       Integer Expected leaf count; anything else exports empty.
#   :handle_leaf      Proc    ->(config, leaf) { true/false }.
#
# NAMING CONVENTION:
...
# -----------------------------------------------------------------------------
    ...
    # FUNCTION | Build the Full Entity List for One Product Config
    # ------------------------------------------------------------
    # @param product [Hash] Product contract (see file header)
    # @param config  [Hash] Live door configuration
    # @return [Array<Hash>] Frozen DXF entity descriptors
    def self.na_build_entities(product, config)
        leaves = product[:leaf_resolver].na_resolve(config)
        return [] unless leaves.length == product[:leaf_count]

        elevation = []
        dimensions = leaves.first[:dimensions]
        na_add_frame(elevation, config, dimensions)
        leaves.each do |leaf|
            panel = product[:panel_resolver].na_resolve(config, leaf)
            na_add_leaf_elevation(elevation, product, config, leaf, panel)
        end
        na_add_dimensions(elevation, product, config, leaves, dimensions) unless config['show_dimensions'] == false
        na_shift_elevation_for_cill(elevation, config, dimensions)

        output = elevation
        na_add_plan_output(output, product, config, leaves, dimensions)
        output.freeze
    end
    # ---------------------------------------------------------------

    # FUNCTION | Export a Product Config as ASCII DXF or Into a Writer
    # ------------------------------------------------------------
    def self.na_export(product, config, writer = nil)
        entities = na_build_entities(product, config)
...
# -----------------------------------------------------------------------------
# REGION | Internal Helpers - Elevation
# -----------------------------------------------------------------------------
    ...
    def self.na_fixed_panels?(product, config)
        GeometryHelpers.na_boolean(config, "#{product[:prefix]}_fixed_panels", false)
    end
    ...
    def self.na_add_leaf_elevation(output, product, config, leaf, panel)
        ...
        return if na_fixed_panels?(product, config)
        na_add_handle(output, product, config, leaf) if product[:handle_leaf].call(config, leaf)
    end
    ...
    def self.na_add_handle(output, product, config, leaf)
        backset = GeometryHelpers.na_number(config, "#{product[:prefix]}_handle_backset_mm", 40)
        height = GeometryHelpers.na_number(config, "#{product[:prefix]}_handle_height_mm", 900)
    ...
    def self.na_add_dimensions(output, product, config, leaves, dimensions)
        layer = na_layer(config, :dimensions)
        output << {
            :type => :text, :layer => layer, :x => dimensions[:width_mm] / 2.0,
            :y => dimensions[:height_mm] + 100, :height => 35,
            :text => "#{product[:label]} #{dimensions[:width_mm].round} x #{dimensions[:height_mm].round} mm"
        }
        leaves.each do |leaf|
            output << {
...
# -----------------------------------------------------------------------------
# REGION | Internal Helpers - Plan
# -----------------------------------------------------------------------------

    def self.na_add_plan_output(output, product, config, leaves, dimensions)
        ...
        leaves.each do |leaf|
            ...
            next if na_fixed_panels?(product, config)
            if GeometryHelpers.na_boolean(config, "#{prefix}_create_open_state_copy", true)
                na_add_open_leaf_plan(output, config, leaf, plan_offset)
            end
            if GeometryHelpers.na_boolean(config, "#{prefix}_show_swing_arcs", true)
                output << {
                    :type => :arc,
...
# -----------------------------------------------------------------------------
# REGION | Internal Helpers - Primitives + Emission
# -----------------------------------------------------------------------------
