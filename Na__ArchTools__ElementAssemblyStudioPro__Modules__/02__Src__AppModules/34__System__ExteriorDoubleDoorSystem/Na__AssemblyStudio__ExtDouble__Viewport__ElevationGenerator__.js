...
const Na__ExtDouble__ElevationGenerator = (function () {
    ...
    function na_palette(config) {
        return {
            ...
            leaf   : na_material_hex(config.double_door_leaf_material_id, 'NA_FRAME_FINISH_SWATCHES', '#b58a58'),
            handle : na_material_hex(config.double_door_handle_material_id, 'NA_HANDLE_FINISH_SWATCHES', '#c0ae8a')
        };
    }
    // ---------------------------------------------------------------
    ...
    function na_draw_glaze_bars(svg, layout, leaf, region, palette) {
        const settings      = leaf.settings;
        const removedSource = Array.isArray(layout.config.double_door_removed_glazebars)
    ...
    function na_draw_leaded_glass(svg, layout, leaf, region) {
        ...
        const disabledSource = Array.isArray(config.double_door_leaded_disabled_cells)
            ? config.double_door_leaded_disabled_cells
    ...
    function na_draw_handle(svg, layout, leaf, palette) {
        const backset = Number(layout.config.double_door_handle_backset_mm) || 40;
        const height  = Number(layout.config.double_door_handle_height_mm) || 900;
        ...
        const towardMeeting = leaf.side === 'left' ? 1 : -1;
    ...
    function na_render(svgElement, config) {
        ...
        const resolved = window.Na__ExtDouble__LeafConfigResolver.na_resolve(config || {});
