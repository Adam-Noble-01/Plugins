# =============================================================================
# ELEMENT ASSEMBLY STUDIO PRO - BIFOLD DOOR PANEL FUSE PARTS
# =============================================================================
#
# FILE       : Na__AssemblyStudio__ExtFold__FuseParts__Panel__.rb
# NAMESPACE  : Na__AssemblyStudio::Na__ExteriorMultiFoldingDoorSystem::Na__FuseParts__Panel
# AUTHOR     : Noble Architecture
# PURPOSE    : Per-leaf timber-frame fusion for bifold doors plus per-leaf
#              glaze-bar fusion, glass trim, AND outer frame fusion at the
#              ADR level so a fully fused bifold door reads exactly like a
#              fused window casement assembly.
#
# DESCRIPTION:
# - Bifold panels are composed inside MOD groups so the inner timber +
#   glazing + glaze-bar groups are children of the MOD, not direct
#   children of the ADR definition. The fuser therefore walks the
#   parent entities, drops into every MOD it finds, and fuses inside
#   that MOD's own entities. The MOD wrapper itself (which carries the
#   ROT/MVE animation contract in its name) is left untouched.
...
# - Frame fusion is intentionally last: the cill is intentionally left
#   as its own group because users may swap its material independently.
#
# DEVELOPMENT LOG:
...
# -----------------------------------------------------------------------------
    ...
    # FUNCTION | Run the Full Bifold FuseParts Pipeline
    # ------------------------------------------------------------
    # Walks the ADR ComponentDefinition entities, locates every MOD
    # group (e.g. `MOD001__ROT__-90-Deg__BifoldPanel`), fuses the timber
    # + glaze bars per leaf, trims the glass with the fused bars, and
    # finally fuses the static outer frame parts at the ADR level so
    # the jamb/head/bottom joint lines disappear.
    #
    # @param entities [Sketchup::Entities] ADR definition entities
    # @return [Hash] { :fused => Int, :failed => Int, :skipped => Int }
    def self.na_fuse_bifold_panels(entities)
        result = { :fused => 0, :failed => 0, :skipped => 0 }
        return result unless entities

        DebugTools.na_debug_method("ExtFold::FuseParts__Panel.na_fuse_bifold_panels")

        mod_groups = na_collect_mod_groups(entities)
        if mod_groups.empty?
            DebugTools.na_debug_geometry("ExtFold fuse: no MOD groups found, skipping leaf fuse")
        else
        ...
        DebugTools.na_debug_geometry(
            "ExtFold fuse summary: fused=#{result[:fused]}, failed=#{result[:failed]}, skipped=#{result[:skipped]}"
        )
        result
    rescue StandardError => e
        DebugTools.na_debug_error("ExtFold::FuseParts__Panel.na_fuse_bifold_panels failed", e)
        result
    end
    # ---------------------------------------------------------------
...
# -----------------------------------------------------------------------------
    ...
    def self.na_fuse_timber_for_panel(entities, panel_id)
        ...
        DebugTools.na_debug_geometry("ExtFold fuse timber: #{panel_id} - merging #{groups.length} parts")
        fused = FuseShared.na_sequential_outer_shell(groups, "Na_DoorPanel_#{panel_id}_Fused")
        if fused
            result[:fused] += 1
        else
            result[:failed] += 1
            DebugTools.na_debug_error("ExtFold fuse timber: #{panel_id} fusion returned nil")
        end
        result
    end
    ...
    def self.na_fuse_glaze_bars_for_panel(entities, panel_id)
        ...
        if groups.length < 2
            DebugTools.na_debug_geometry("ExtFold fuse bars: #{panel_id} has <2 bars, skipping")
            result[:skipped] += 1
            return result
        end

        DebugTools.na_debug_geometry("ExtFold fuse bars: #{panel_id} - merging #{groups.length} bars")
        fused = FuseShared.na_sequential_outer_shell(groups, "Na_GlazeBar_#{panel_id}_Fused")
        if fused
            result[:fused] += 1
        else
            result[:failed] += 1
            DebugTools.na_debug_error("ExtFold fuse bars: #{panel_id} fusion returned nil")
        end
        result
    end
    ...
    def self.na_trim_glass_for_panel(entities, panel_id)
        ...
        unless bar_group.manifold? && glass_group.manifold?
            DebugTools.na_debug_warn("ExtFold trim glass: #{panel_id} non-manifold operand(s); skipping")
            result[:failed] += 1
            return result
        end
        ...
        begin
            ...
            if trimmed
                ...
                result[:fused] += 1
                DebugTools.na_debug_geometry("ExtFold trim glass: #{panel_id} -> #{trimmed.name}")
            else
                result[:failed] += 1
                DebugTools.na_debug_error("ExtFold trim glass: #{panel_id} returned nil")
            end
        rescue StandardError => e
            result[:failed] += 1
            DebugTools.na_debug_error("ExtFold trim glass: #{panel_id} raised", e)
        end
...
# -----------------------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # The bifold frame is created via the WindowSystem GeometryBuilders
    ...
    def self.na_fuse_outer_frame(parent_entities)
        ...
        if groups.length < 2
            DebugTools.na_debug_geometry("ExtFold fuse frame: <2 parts found, skipping")
            result[:skipped] += [groups.length, 1].max
            return result
        end

        DebugTools.na_debug_geometry("ExtFold fuse frame: merging #{groups.length} parts")
        fused = FuseShared.na_sequential_outer_shell(groups, "Na_Frame_Fused")
        if fused
            result[:fused] += 1
        else
            result[:failed] += 1
            DebugTools.na_debug_error("ExtFold fuse frame: returned nil")
        end
        result
    end
...
# -----------------------------------------------------------------------------
    ...
    # HELPER FUNCTION | Collect All MOD Groups in a Set of Entities
    # ------------------------------------------------------------
    ...
    # HELPER FUNCTION | Discover Unique Panel IDs From a Set of Entities
    # ------------------------------------------------------------
