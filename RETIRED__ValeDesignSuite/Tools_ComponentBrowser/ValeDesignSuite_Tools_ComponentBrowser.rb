...
#
# 27-Jul-2025 - Version 1.2.0
# - Added a new regex pattern to clean up component names with "__" after the series number
#  - Example `10-Series__xyz``
#
# =============================================================================
...
# -----------------------------------------------------------------------------
    ...
    PAPER_PREFIXES_TO_EXCLUDE = [
        ...
        'A4-Paper__',
        'A5-Paper__'
    ]                                                                              # <-- Folder prefixes to exclude from browser
...
# -----------------------------------------------------------------------------
    ...
    def self.should_exclude_folder?(folder_name)
        ...
        return true if folder_name.end_with?(EXCLUDED_FOLDER_SUFFIX)               # <-- Check for excluded suffix
        # Exclude folders starting with any of the paper prefixes
        return true if PAPER_PREFIXES_TO_EXCLUDE.any? { |prefix| folder_name.start_with?(prefix) } # <-- Check for paper prefixes
        false
    end
    # ---------------------------------------------------------------
...
# -----------------------------------------------------------------------------
    ...
    def self.clean_display_name(original_name)
        ...
        # Pattern 0: "01__" prefix (specific for paper components)
        cleaned = cleaned.gsub(/^01__/, '')                                      # <-- Remove "01_" prefix
        ...
        # Pattern 6: Series prefixes like "00-Series_-_", "10-Series_-_", "10-Series__", etc.
        cleaned = cleaned.gsub(/^\d{2}-Series[-_]*/, '')                        # <-- Remove number-Series Divider Suffix Type1
        cleaned = cleaned.gsub(/^\d{2}-Series[__]*/, '')                        # <-- Remove number-Series Divider Suffix Type2
        
        # Pattern 7: Other common prefixes like "ABC_123_" or "ABC_123"
