...
require 'TT_Lib2/c_extension_manager.rb'
...
MSG
  ...
  hex_id_bytes = TT::System::PLATFORM_BITNESS == 8 ? 16 : 8
  # @private
  OBJECT_ID_FORMAT = "0x%.#{hex_id_bytes}x".freeze

  # @private
  def self.object_id_hex_legacy( object )
    OBJECT_ID_FORMAT % (object.object_id << 1)
  end

  # @private
  def self.object_id_hex_impl( object )
    OBJECT_ID_FORMAT % self.object_address( object )
  end

  # @!method object_id_hex(object)
  # @!scope class
  #
  # @note As of version 2.15.0 this returns the same hex valueas Ruby.
  #   Before that it was relying on implementation detail in Ruby that
  #   `Object#object_id` was based on the memory address of the Ruby object.
  #   That changed as of Ruby 2.7. Another adjustment made with 2.15.0 was to
  #   pad the value zeroes.
  #
  # @param [Object] object
  #
  # @return [String]
  ...
  # Dynamically alias to the right implementation.
  # This is done because it's hard to build the C extension for the older Ruby
  # versions so the "legacy" implementation is kept as a fallback.
  class << self
    if RUBY_VERSION.to_f < 2.7 && !respond_to?(:object_address)
      alias :object_id_hex :object_id_hex_legacy
    else
      alias :object_id_hex :object_id_hex_impl
    end
  end
...
if TT::System.platform_supported?
  ...
  message = "#{TT::Lib::PLUGIN_NAME} is not supported for this platform."
  UI.messagebox(message)
